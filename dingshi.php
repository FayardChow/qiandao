...
while ($row = mysql_fetch_array($re)) {
	//$CQ->sendGroupMsg($qunhao, $row['add_time']);
    ...
    $over_time = $min - $all_min;  //超时时间
  	...
  	if($over_time >= -5 && $over_time <= 0) {
  		$CQ->sendGroupMsg($qunhao, $CQ->cqAt($row['qq'])." ".$row['name'].$row['item']."还有".abs($over_time)."分钟, 请尽快签回");
  	}


  	if($over_time == 5) {
  		$CQ->sendGroupMsg($qunhao, $CQ->cqAt($row['qq']).$row['name'].$row['item']."已超时5分钟,系统将不再提示");
  	}
    

}

// $CQ->sendGroupMsg($qunhao, "定时任务");
