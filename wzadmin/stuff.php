...
//清空所有
elseif($my=='qk'){
  $sql=$DB->query("DELETE FROM stuff");
  if($sql){$res='删除成功！';}
  else{$res='删除失败！';}
  exit("<script language='javascript'>alert('{$res}');history.go(-1);</script>");
...
if(isset($_GET['kw'])) {
	...
	$numrows=$DB->count("SELECT count(*) from stuff WHERE{$sql}");
	...
	if($_GET['qq'] != "" && $_GET['name'] != "") {
		...
		if(preg_match("/\d{6,}/", $qq)) {
			...
			if($nrows) {
				...
				$re = $DB->query("INSERT INTO stuff (qq, name) VALUES ('$qq', '$name')");

				if($re) {
					$DB->query("UPDATE log SET name='$name' WHERE qq='$qq'");  //更新原有记录名称
	...
	$numrows=$DB->count("SELECT count(*) from stuff WHERE 1");
	$sql=" 1";
